...
ensure_package("rich", REQUIRED_PACKAGES["rich"])
    ...
    def collect(self) -> List[Dict[str, Any]]:
        ...
        for hive, path in self.UNINSTALL_KEYS:
            try:
                ...
                continue
            count = safe_call(winreg.QueryInfoKey, default=(0, 0, 0), key)[0]
            for index in range(count):
                try:
                    subkey_name = winreg.EnumKey(key, index)
                    subkey = winreg.OpenKey(key, subkey_name)
                    name = safe_call(winreg.QueryValueEx, default=(None, None), subkey, "DisplayName")[0]
                    if not name:
                        continue
                    version = safe_call(winreg.QueryValueEx, default=(None, None), subkey, "DisplayVersion")[0]
                    publisher = safe_call(winreg.QueryValueEx, default=(None, None), subkey, "Publisher")[0]
                    install_date = safe_call(winreg.QueryValueEx, default=(None, None), subkey, "InstallDate")[0]
                    programs.append({
                        "name": name,
                        "version": version or "-",
...
class Dashboard:
    ...
    def build_cpu_panel(self, specs: Dict[str, Any], metrics: Dict[str, Any]) -> Panel:
        ...
        table.add_row("Global usage:", f"{overall:.1f}%")
        ...
        return Panel(Group(table, Text(percent_bar(overall) + f" {overall:.1f}%\n"), Text(spark), core_table), title="CPU Monitoring", box=box.ROUNDED, border_style="magenta")
    ...
    def build_memory_panel(self, memory_info: Dict[str, Any]) -> Panel:
        ...
        return Panel(Group(table, Text(percent_bar(percent) + f" {percent:.1f}% - {status}")), title="Memory Monitoring", box=box.ROUNDED, border_style="green")
    ...
    def build_network_panel(self, interfaces: List[Dict[str, Any]], throughput: Dict[str, float], public_ip: str) -> Panel:
        ...
        for iface in interfaces[:6]:
            table.add_row(
                ...
                f"{safe_int(iface.get('speed'))} Mbps",
            )
        traffic = Text(f"Public IP: {public_ip} | Upload: {human_bytes(throughput.get('sent_rate', 0))}/s | Download: {human_bytes(throughput.get('recv_rate', 0))}/s")
        return Panel(Group(table, traffic), title="Network Audit", box=box.ROUNDED, border_style="bright_blue")

    def build_gpu_panel(self, gpus: List[Dict[str, Any]]) -> Panel:
        if not gpus:
            return Panel("GPU not detected or GPUtil unavailable", title="GPU", box=box.ROUNDED, border_style="red")
    ...
    def build_score_panel(self, diagnostics: Dict[str, Any], benchmark: Dict[str, Any], security: Dict[str, Any], network_score: int, global_grade: str) -> Panel:
        ...
        summary.add_column(ratio=1)
        summary.add_row("System:", f"{score}/100")
        summary.add_row("Grade:", global_grade)
    ...
    def run(self, once: bool = False, report_format: Optional[str] = None) -> None:
        ...
        layout["footer"].split_row(Layout(name="security"), Layout(name="scores"), Layout(name="ports"))
...
Path('d:/Petit projet python/system_monitor.py').write_text(new_code, encoding='utf-8')
